	...
	$.getJSON("current.json?"+Math.random(), function(obj) {
		var myVal = obj.startingValue;
				$.each(obj.entries, function(key, value) {
					...
					$('#list').append("<tr class='myBorderColor'><td style='border: none;' class='myBGColor'>$"+value.amount+"</td><td style='border: none;' class='myBGColor'>"+value.category+"</td>");
				});

				$('#total').append("<h1 class='myBGColor3 text-center'>$"+myVal.fart.toFixed(2)+"</h1>");		
